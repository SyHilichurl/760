...
# Obtain five images from the test loader
...
# Loop over five images
for img_idx in range(5):
    input_img = images[img_idx].unsqueeze(0)
    input_img.requires_grad = True

    # Perform a forward pass through the model
    outputs = model(input_img)
    outputs = torch.nn.functional.softmax(outputs, dim=1)  # Convert to probabilities

    # Set up the plot for this image
    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
    image_np = input_img.detach().cpu().squeeze().numpy().transpose(1, 2, 0)
    image_np = (image_np * np.array([0.229, 0.224, 0.225])) + np.array([0.485, 0.456, 0.406])  # 反归一化
    image_np = np.clip(image_np, 0, 1)
    axes[0].imshow(image_np)
    axes[0].set_title(f'Original Image {img_idx + 1}')
    axes[0].axis('off')

    # Perform a backward pass for each class and visualize the saliency maps
    for i, class_name in enumerate(['Sad', 'Angry', 'Happy']):
        # Reset gradients to zero
        model.zero_grad()
        if input_img.grad is not None:
            input_img.grad.data.zero_()

        class_score = outputs[0, i]
        class_score.backward(retain_graph=True)
        
        
        if input_img.grad is not None:
            saliency = input_img.grad.data.abs().squeeze().sum(0)
            saliency = (saliency - saliency.min()) / (saliency.max() - saliency.min())
            axes[i + 1].imshow(saliency.cpu().numpy(), cmap='hot')
            axes[i + 1].set_title(f'Saliency Map for {class_name}')
            axes[i + 1].axis('off')
        else:
            print(f"No gradient for {class_name}; check model connectivity and operations.")
    plt.show()
